            ...
            <div class="col-md-6 col-xl-12">
              <form method="POST" enctype="multipart/form-data" id="my-form">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" name="nama_jenis_pekerjaan" id="nama_jenis_pekerjaan" class="form-control onlytext" >
                </div>
...
<script> 
  ...
  function DeleteData(id){ 
    ...
    function UbahData(id){
        ...
        $.ajax({
          url:  "{{ route('jeniskerja_put') }}",
          data: {id:id},
          type: "POST",
          success: function(result){
            console.log(result);
            $id = $("#id").val(result.id);
            $nama_jenis_pekerjaan = $("#nama_jenis_pekerjaan").val(result.nama_jenis_pekerjaan);   
          }
        })
        $('#modal-large').modal({ show: true });
    }
  ...
  $(function () { 
      var table = $('#example').DataTable({
          processing: true,
          serverSide: true,
          ajax: "{{ route('jeniskerja_all_data') }}",
          columns: [
              {data: 'nama_jenis_pekerjaan', name: 'nama_jenis_pekerjaan'},
