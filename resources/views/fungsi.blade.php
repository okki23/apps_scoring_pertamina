...
<div class="card card-statistics widget-blog-contant">
  <div class="card-header">
      <div class="card-heading">
          <h4 class="card-title">Fungsi Pekerjaan</h4>
      </div>
  </div>
  <div class="card-body">
          ...
          <div class="modal-body">
            <div class="col-md-6 col-xl-12">
              <form method="POST" enctype="multipart/form-data" id="my-form">
                ...
                <div class="mb-3">
                  <label class="form-label">Nama Fungsi Pekerjaan</label>
                  <input type="text" name="nama_fungsi_pekerjaan" id="nama_fungsi_pekerjaan" class="form-control onlytext" >
                </div>
...
<script> 
  ...
  function DeleteData(id){ 
    ...
    function UbahData(id){
        ...
        $.ajax({
          url:  "{{ route('fungsi_put') }}",
          data: {id:id},
          type: "POST",
          success: function(result){
            console.log(result);
            $id = $("#id").val(result.id);
            $nama_fungsi_pekerjaan = $("#nama_fungsi_pekerjaan").val(result.nama_fungsi_pekerjaan);   
          }
        })
        $('#modal-large').modal({ show: true });
    }
  ...
  $(function () { 
      var table = $('#example').DataTable({
          processing: true,
          serverSide: true,
          ajax: "{{ route('fungsi_all') }}",
          columns: [
              {data: 'nama_fungsi_pekerjaan', name: 'nama_fungsi_pekerjaan'},
